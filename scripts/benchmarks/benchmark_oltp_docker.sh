#!/usr/bin/env bash
# Docker MySQL OLTP Benchmark (baseline - no Kata, no Firecracker)
# 5-run sysbench oltp_read_write for direct comparison with Kata+FC benchmarks
set -euo pipefail

# --- Configuration ---
CONTAINER="mysql-bench-docker"
...
WARMUP=30
REPORT_INTERVAL=10
RESULTS_DIR="./bench_docker_$(date +%Y%m%d_%H%M%S)"
...
log "Mode: Docker (no Kata, no Firecracker — baseline)"
...
# Cleanup trap
DATA_VOLUME="mysql-bench-data"
cleanup_on_exit() {
    docker rm -f "$CONTAINER" 2>/dev/null || true
    docker volume rm "$DATA_VOLUME" 2>/dev/null || true
    log "Cleaned up Docker container and data volume"
}
...
# Create a named volume to bypass overlay2 for MySQL data (direct host filesystem I/O)
docker volume rm "$DATA_VOLUME" 2>/dev/null || true
docker volume create "$DATA_VOLUME" >/dev/null
log "Created Docker volume '$DATA_VOLUME' (bypasses overlay2 for data I/O)"
...
log "MySQL tuned: 2G buffer pool, O_DIRECT flush, innodb_io_capacity=2000"
...
**Storage:** ${DETECTED_STORAGE} + Docker volume (bypasses overlay2 for data) | **Isolation:** None (shared host kernel)
